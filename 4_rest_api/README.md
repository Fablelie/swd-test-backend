## 🗺️ MVT Architecture Mapping & Directory Structure
```text
┌────────────────────────────────────────────────────────────────────────────────┐
│  [ LAYER ]          │ [ FILE / DIRECTORY ]        │ [ RESPONSIBILITY ]         │
├─────────────────────┼─────────────────────────────┼────────────────────────────┤
│ 🌐 ROUTER LAYER     │  exam_app/urls.py           │  ด่านหน้า: ดักจับ URL Pattern │
│                     │  apis/urls.py               │  และส่งงานต่อให้ View         │
├─────────────────────┼─────────────────────────────┼────────────────────────────┤
│ 🧠 VIEW LAYER       │  apis/views/v1/classroom.py │  หัวใจระบบ: ควบคุม Logic    │
│  (Controller Role)  │  apis/views/v1/school.py    │  (รับ Request, สั่ง Query     │
│                     │  apis/views/v1/student.py   │  ข้อมูล, ประมวลผลผลลัพธ์)     │
│                     │  apis/views/v1/teacher.py   │                            │
├─────────────────────┼─────────────────────────────┼────────────────────────────┤
│ 🗄️ MODEL LAYER      │  apis/models.py             │  จัดการข้อมูล: กำหนด Schema  │
│                     │  apis/filters.py            │  ของ DB และทำ SQL Query    │
├─────────────────────┼─────────────────────────────┼────────────────────────────┤
│ 🎨 TEMPLATE LAYER   │  apis/serializers.py        │  จัดหน้าตาข้อมูล: แปลงจาก     │
│  (API Serializer)   │                             │  Python Object เป็น JSON     │
└─────────────────────────────────────────────────────────────────────────────────┘
```
...
[ Client / หน้าบ้าน ] 
     │  ▲
     │  └── (5) Response: JSON (ส่งกลับโดย serializers.py)
     ▼
┌────────────────────────────────────────────────────────┐
│ (1) ROUTER LAYER (ดักจับ URL ตาม Pattern)                │
│     - exam_app/urls.py  (ทางเข้าหลัก)                    │
│     - apis/urls.py      (แยกเส้นทางเข้าแอป API)          │
└────────────────────────────────────────────────────────┘
     │
     ▼
┌────────────────────────────────────────────────────────┐
│ (2) VIEW LAYER (ตัวควบคุม Logic - คล้าย Controller)       │
│     - apis/views/v1/student.py                         │
│     (รับคำสั่ง, เรียกใช้ Filter, สั่งดึงข้อมูลจาก Model)         │
└────────────────────────────────────────────────────────┘
     │  ▲                                      │
     │  └── ผลลัพธ์ Query                       │  (ส่งต่อข้อมูลดิบ)
     ▼                                         ▼
┌─────────────────────────┐              ┌─────────────────────────┐
│ (3) MODEL LAYER         │              │ (4) TEMPLATE/SERIALIZER │
│     (จัดการข้อมูล/DB)      │              │     (แปลงหน้าตาข้อมูล)    │
│ - apis/models.py        │              │ - apis/serializers.py   │
│ - apis/filters.py       │              │                         │
└─────────────────────────┘              └─────────────────────────┘
```
